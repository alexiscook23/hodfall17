...
we already did that above on line 38 because you just write is as semicolon instead of comma
-when you load it in R, it won't be separated by commas or semicolons because it's nicely formatted; it's only if you're looking at the data on another format like excel etc (or even R studio in the text files) that you'll see it as just numbers separated by whatever you specify
...
```
so that above chunk means that you just created a specific width called my.widths and then defined it. then you create my_positions to be defined by the my.widths you created and then you can go and read the data (renamed the new dataset with these specifications as hsb3) and then read it. it just looks like lines of numbers, but you can go and read it by counting how many across and knowing each it a column. in other words, the first row looks like 0700411157 etc and you know the first row ends at 070 and then the next column is 0 and the next is 4 and the next is 1 etc because the width you specified means that. so 70 is the id, 1 is for female, 1 is for male, 1 is for ses etc... 
...
View(free)
```
all the above means that if the file doesn't exist, then download the file and open it. then get rid of the column names and skip the white space ("col_names=false" and skip=4). the "else" means if you already have the file then just open it and get rid of column names and the white space
-this is just a polite way to not have to redownload the files over and over; it's polite to not have to keep redownload because the people who own the files have programming so that you're limited to how many times you can request to download their files
-also, you can check whether or not the file exists by typing "file.exists("free.xls")" and see if it comes back TRUE or FALSE
...
NOTE: R starts indexes at 1; it never starts at 0
...
View(free2)
```
on 136, we create a new dataset called free2 and get rid of the columns with white space. then on 139 we created a logical statement saying to get rid of rows that don't have anything on column x__1
...
tail(free2)
```
the head and tail commands just show you the data using either top to bottom or bottom to top
...
names(free2)<-c("state",
                ...
                "pc_frl_2012")
```
this renames the columns in your free2 dataset
...
In the first step, I select the state name and the "total" columns. this creates a new variable that is comproside of those 5 variables we asked for
```{r}
...
-this means you can make the data "long" instead of "wide," which is what we want
...
free_total<-free_total%>%arrange(state,year)
```
line 202 makes it so your dataset "free_total" is arranged by state and then year so that all the years for a state are together as opposed to being arranged by year so you have to scroll forever to find each year for a single state
...
View(frl_total)
frl_total<-frl_total%>%arrange(state,year)
```

Now I join these two datasets, using what's called a "left" join. This means that the merge will keep all of the data in the first dataset, and only those rows in the second dataset that match based on state and year, which are specified in the command. The names of the rows in both datasets must be exactly the same so that they can be merged
...
#you can also use the percent of free/reduced lunch students much easier than using the old dataset (free2) by just creating a mutate command that takes the total number of students and divides it by the frl students
```
...
### Stata
Stata is a very popular statistical programming language among economists and social scientists. Stata files are stored in `dta` format and are stored as .dta (instead of .Rmd etc)
...
Most of the time, you should store your data as a csv file. This will ensure that pretty much anyone can take a look at it. If you're sure that the only users will be other R users (which is almost never unless you're in a class), then feel free to save it as an `.Rdata` file. 
...
```
everyone can always view csv, so that's really what you should save things as
