...
```{r}
#create a new node with the number for field goals (guessed 9 because it kept showing up when i clicked on it on the website)
...
simple_data<-tibble(player_name,points,field_goals)
...
get_nba_data<-TRUE #we can turn this to false later so we don't continually request data from the website
...
```{r}
nba_df_all<-nba_df_all%>%mutate(pts_minute=pts/minutes_played) #this creates a new varialbe that is points per minute

gg<-ggplot(filter(nba_df_all,pts>1000,year>2000),aes(y=pts_minute,x=three_pointers, #this plots only players who scored over 1000 points past the year 2000
                          text=player_name,
...
ggplotly(gg) #ggplotly makes the plot interactive
```
...
lookup_code("TN","Davidson") #this lookup_code command is from tigris (all that matters is that it's a third party thing that'll tell u stuff if you downloaded the package)
