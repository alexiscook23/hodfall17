...
So essentially a regression model is the expected value of y as a function of x
---beta0 is the intercept (the y value if x is zero) and b1x1 is basically the slope and then epsilon is how far each individual point is from the regression line (the averages); epsilon is the error term
...
---you can have as many predictors as you want as long as it's less than the number of observations (which it will be cuz you'll have data with thousands of observations)
---each b1x1 is a predictor
...
## You don't need it right now, but I want to show you how it's done
# we have it written out as comments so that we don't all get different answers since if we run this code we'll all be pulling from different random samples
...
```
This graph isn't predictive; it's just us looking at how much people dine out; it's the variation in how much people spend on dining out (so almost 1000 families spend 0, about 100 spend 2000 etc)
...
mod1<-lm(dine_out~fam_size,data=cex) #outcome on left, predictor on right #lm means linear model; we're creating this model named mod1 that's a linear model that shows dining out as a function of family size

summary(mod1) #the residuals show the standard errors; so this summary shows that the median error is -140 meaning we're an average of $140 too low in our estimation
#we're not really interested in the statistical significance of each of the coefficients using t value because it's basically always going to significant (t value greater than 2). Basically it just means the predictive value is not zero. We're more interested in predictive accuracy of our model using rmse
#the estimate std is interesting because it can show some predictive power (like in this case an increase in one child is an increase in $41 in dining expenditure)
confint(mod1)
...
cex<-cex%>%mutate(pred1=predict(mod1)) #basically just adding another variable in the dataset (using mutate) where it's the predictive value of y with every value of x (predict using data in memory)
 
rmse_1<-with(cex,rmse(dine_out,pred1)); rmse_1
#this says that with the dataset called cex use the variables called dine_out and pred1 create rmse (the semicolon and then rmse_1 just means show me the prediction after you create it)
...
---this isn't a great predictor because there's huge variation around the trendline. Also, the data really thins out after 5 (because few families have over 5 children)

_Quick Exercise_ Run a regression using a different predictor. Calculate rmse and see if you can beat my score. 
```{r exercise}
#chose to use poverty level as a predictor of dining out expenditure; was a better predictor than family size based on the rmse
mod2<-lm(dine_out~pov_cym,data=cex) 
...
```
Quick note: if you used a categorical variable like income level, then it'll be based off a reference point (the first income level, meaning the most impoverished) and then each standard thing after will mean "how much more does this income level spend on dining out than the reference level" but you need to watch statistical significance using t value for those cuz it could actually be equal to zero (meaning there is no difference between expenditure of the different income levels)... I didn't use a categorical variable tho because poverty was binary in this case
## Multiple Regression. 
...
g2
#this graph shows that the idea that those who spend more on groceries would spend less on dining out is totally wrong; those who spend more just spend more

cex<-cex%>%mutate(pred4=predict(mod4))

rmse_4<-with(cex,rmse(dine_out,pred4));rmse_4

```
What we're seeing with the trendline in the graph is that regressions are heavily skewed by outliers and most of the data is clumped in the corner because people really aren't spending that much on dining; would be better to make it proportional and ask how much they're spending as a proportion of their income etc or even better is to take the log transform
...
mod5<-lm(log(dine_out+1)~ #log of dining out, plus one for zeros
...
summary(mod5)
#so this is the model of the log of the outcome instead of the actual outcome
#the estimate in the summary is on a percent scale, so it means that for every 1 percent increase in dining out, there's a .4 percent increase in grocery spending
cex<-cex%>%mutate(pred5=predict(mod5))

rmse_5<-with(cex,exp(rmse(log(dine_out+1),pred5)));rmse_5
#so we just put the output on the log scale and then took it back into normal numbers; the command to take it back is exponentiate (called "exp")
#this is way, way more accurate of a prediction. We went from an rmse of 577 to 12. You'll know you need to transform with log if your data looks super skewed like ours did in the beginning 

g5<-ggplot(cex, aes(x=grocery,y=exp(pred5),color=pov_cym))
g5<-g5+geom_point(shape=1)

g5

# Function defined by coefficients

fun_mod5<-function(x) exp(mod5$coefficients[1]+ 
                          (mod5$coefficients[2]*log(x+1))+
                          (mod5$coefficients[3]*1)+
                          (mod5$coefficients[4]*mean(cex$fam_size,na.rm=TRUE))  
                          ) 
