...
mod4_predict<-predict(mod4)

##testing data
test_dine_out<-cex_test$dine_out

##create new dataset that compares mod4 and the testing data but only the dining out variable
compare_data<-data.frame(mod4_predict,test_dine_out)

compare_data<-compare_data%>%filter(is.na(mod5_predict)==FALSE)

mod4_predict<-predict(mod4,newdata=cex_test)


##can  see the summary of mod4 without manipulating the data and take the exponent of residual error for in-sample rmse
summary(mod4)
...
#the rmse won't work whenever you do na.action="na.exclude", but you can still see the standard error on the summary (residual standard error). The difference is that this standard error is based on the logs in your model, so you need to take the exponent of that number. For examply, my standard error was 2.05, so I take exp(2.05) to find out the actual rmse of this model
##---the issue is that this will only be the rmse for the in-sample data; to get the rmse for out-of-sample data, you'll have to manipulate the data
...
g4a
```
R squared is the percent of the model that is affected by the variables you chose; you want it to go up to increase accuracy. But you want RMSE to go down because that means the variation in your model is going down

You take the log of any variable within your model if the variable is highly skewed (in our example, groceries and dining out were both super right skewed so we took the variable when adding them into the model)

If you have any missing data in your datasets/variables, R will skip the entire variable and give you an error message. Because of this, you need to write na.action="na.exclude" at the end of your command message
...
---we have the code to split the data in half up in lines 42 and down (it's commented out)
...
#try comparing rmse with different models you have above; the rmse of model 3 is terrible because there were no logs in that model
rmse_5_test;rmse_3

rmse_5_test;rmse_4
#the rmse of model 4 is NA because of missing data in the model

```
Why is the value from the testing dataset so much larger? 
--they're just different samples so they have different rmse; it should raise eyebrows that our rmses are so different tho... means we're not as accurate as we thought. the key to making predictions is to try and make a model that will be good at making sample data (the difference in rmse should show us that we're not that great at this point)
--the trick is to get enough predictors in there that you have a good prediction but also not too many predictors where it's overfit only to the half of the data that youre using

#run rmse using the out of sample data
...
----you should know that regressions on binary variables will be a little wonky regarding the summary, but it can still be good as an intermediary assessment
...
*****Go look on 05-regression.Rmd because he added extra stuff about how to use the linear regression model to make actual predictions (multiply the coefficients and then add them all together and take the exponent, since it was a log for the regression, and that answer is your actual prediction)
