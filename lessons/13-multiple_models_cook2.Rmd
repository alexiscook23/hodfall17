...
#this whole chunk is about to do automatically what we did by hand with the cross-validation models before
qf<-qf%>%
...
fit1$results #this gives rmse and rmsesd (which tells u variability of all your resamples aka the 1000 folds of the model that you ran)
names(fit1) #this gives the names of the things within the caret package
...
qf<-qf%>%select(-median_home_val) #deselect median home value cuz you don't want to predict the log of median home value with median home value (that'd be dumb)

names(qf)

fit2<-train(log_median_home_val~., #the period means use whatever coefficients are best (tells the computer it can choose; this means everything in your dataframe must be numeric)
            data=qf,
            method="glmStepAIC", #the AIC method penalizes the model for each additional variable (kind of like a more sophisticated r-squared value)
            trControl=fitControl)

fit1$results

fit2$results #this machine learning dropped the rmse considerably (down to .15) and dropped the rmsesd (dropped the variability)

summary(fit2) #you need to check these results because there could be illogical things (the computer has no context, so it only tells u the numbers)
