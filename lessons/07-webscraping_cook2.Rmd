...
select_zip<-geo.make(zip.code=ziplist) #geo.make command puts together a well-formatted request to send to the api (using the dataset we had already created)

county_educ=acs.fetch(geography=select_zip, #the acs.fetch command is how you communicate with the acs
                      endyear=2014,
                      table.number="B15002",
                      col.names="pretty",verbose=T) #the pretty command just means it'll come back as something intelligible and legible
...
prop_coll_above<-divide.acs(numerator=(county_educ[,15]+
                                      county_educ[,16]+
                                      county_educ[,17]+
                                      county_educ[,18]+ #so this makes the numerator all the males and females and the denominator is total number in county
                                      county_educ[,32]+
                                      county_educ[,33]+
                                      county_educ[,34]+
                                      county_educ[,35]),
...
#you can figure out what's in a package by typing the command library(help="that package") and then it'll pull up all the commands you can use. This is major key (Ex: library(help="acs"))
