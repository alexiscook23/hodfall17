...
R can understand a web connection via the `url` command. Once that connection is established, we can download whatever we'd like. So basically the url command downloads any webpage onto R
...
marathon<-marathon%>%mutate(Nationality=fct_reorder(f=as.factor(Nationality),x=-speed,fun = max)) #this code reorders factors based on speed and by the fastest speed from that nationality (so it puts the nationalities in order of who ran the fastest from each nationality)
...
# Get the page, pull the tables via html_table
marathonf <- read_html(marathon_wiki)%>%html_table()

#women's table
marathonf<-tbl_df(data.frame(marathonf[[2]])) #put a 2 to ask for the second table on this webpage

head(marathonf)

#Convert time to seconds
marathonf<-marathonf%>%mutate(Time2=hms(as.character(Time)))%>%mutate(Time2=period_to_seconds(Time2))

#Marathons are 42,200 meters long
marathonf$speed<-(4.22e4)/marathonf$Time2

#Get dates in a usable format usin lubridate::mdy
marathonf$date<-mdy(marathonf$Date)


marathonf<-marathonf%>%mutate(Nationality=fct_reorder(f=as.factor(Nationality),x=-speed,fun = max)) #this code reorders factors based on speed and by the fastest speed from that nationality (so it puts the nationalities in order of who ran the fastest from each nationality)

g1<-ggplot(data=marathonf,
           aes(y=speed,x=marathonf$date,
               #Reorder nationality by fastest times
               color=Nationality,
               text=paste(Name,Time))
           )  
...
player_name<-page%>%
  html_nodes(my_nodes)%>% #html_nodes helps us pull out the specific parts of the webpage we want
  html_text()
...
```{r}
## Get data frame started: fill in player names
my_nodes<-paste0(".right:nth-child(",30,")") #so we figured out that row 30 is the points column by trail and error (just put in a number and see what it pulls up and then you can figure it out); the number won't always be the column number tho... it can be any piece of the website
...
```{r}
player_name<-player_name[-length(player_name)] #this says cut the last element of this list (because it's an ad)

<<<<<<< HEAD
simple_data<-tibble(player_name,points) #tibble is easier to use in R than a normal dataframe, but it's basically just another type of dataframe format
=======
simple_data<-tibble(player_name,points,fg)
>>>>>>> 14491df954fd1a49de79226adb963bfeea71f51c
```
...
```{r}
## Get data frame started: fill in player names
my_nodes<-paste0(".left:nth-child(",9,")")
...
We're going to interact directly with the Zillow api, as opposed to using an R "wrapper" like "acs.R". This means we have to have a properly formatted request AND be able to interpret the results when they come back. These requests generally come back in a coding language we don't understand; that's okay. All you need to know is how to extract the values we're asking for
...
zdemodata<- list(zip = list(as.character()),
                ...
                ) #to find these names, you just need to search in the file it gives you and find the name of what you're looking for

#repeat across all zip codes
for (i in 1:length(ziplist)) { #this i in 1 creates a loop where it says start with the first element and then go to then end

  # url needs zillow key and zip code
  url=paste0("http://www.zillow.com/webservice/GetDemographics.htm?zws-id=",zwsid,"&zip=",ziplist[i]) #the i in the brackets means just keep doing this request for all of the elements until you get to the end (last zip code in the list) #r substitutes my api code from the ,swsid,
  #so you just put this url into your web-browser to see the code and then find the names of the elements you want
  ...
  #exersize: Look for median 4 bedroom home value and pull it
...
head(zdemodata)
#so what you need to get out of this is that if there's a dataset out there, then you can use an api and turn that into a dataframe in r... it may require a bit of work, but it's definitely do-able
