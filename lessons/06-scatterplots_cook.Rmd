...
##the predictions from the model are the conditional means
```
...
-This scatterplot really doesn't show any patterns and the x and y axes aren't clear (would have to rename to be clearer)

```{r}
g1<-g1+geom_smooth(method="lm")
g1<-g1+xlab("Income Rank")+ylab("Grocery Spending")
g1
```
...
##so basically we can now graph the average of each income rank, meaning there'll only be 100 dots (one for each income rank 0-100). This also gets rid of outliers because we're just averaging each rank

g1a<-ggplot(cex_sum,aes(x=inc_rank_r,y=groc_mean))

g1a<-g1a+geom_point()

g1a<-g1a+xlab("Income Rank")+ylab("Grocery Spending")
...
Notice how different the steps are on the y axis now (exponential). But this shows that our line actually fits the data much better once we work on the log scale. We're now ready to go ahead and run our first model based on what we've learned. 
...
#shows that the relationship is highly stat significant and for every 1% increase in income there's a 
```
...
```{r}
m2<-lm(log(other_store+1)~inc_rank,data=cex);summary(m2)

cex_sum1<-cex%>%mutate(inc_rank_r=round(inc_rank))%>%group_by(inc_rank_r)%>%summarize(store_mean=mean(other_store,na.rm=TRUE))

#na.rm means remove the missing data

g2<-ggplot(cex_sum1,aes(x=inc_rank_r,y=store_mean))

g2<-g2+geom_point()

g2<-g2+xlab("Income Rank")+ylab("Other Store Expenditures")
...
#other ways to differentiate data is by color of the dot and size of the dot; this time we're using the color of the dot (to show age of children in household)
...
g2<-g2+xlab("Income Rank")+ylab("Grocery Spending")
g2

```

Our first graphic is a bit complex, but shows that those in the "0" category, with no kids, have consistently lower spending on groceries across income levels. So let's drop that group and see what the graphic looks like for those with at least one kid of any age. (basically just shows that the red dot aka no kids spends less on groceries)
...
hypo.data<-data.frame(expand.grid(
          ...
          childage=levels(cex$childage)                                     
                          ...
                          ) #so that basically just said go from the minimum of the income rank to the max of income rank in 100 steps, so it just figures out the gap it needs between each and creates that data (doesn't even need to take the average or anything)
head(hypo.data)
View(hypo.data)
```
so the above created a new dataset using the existing dataset and it ranked the income levels from 0-100 using every category of childage (so 800 total observations cuz 8 categories of childage)
...
hypo.data$groc.lwr<-exp(hypo.data$lwr) #exponentiate lower boundary (adds in the lower and upper confidence intervals)
...
g4
```
To show this in the data we can break it out for every type of child age grouping. Notice we didn't plot the actual data; we plotted predicted values based on a different dataset that we created
