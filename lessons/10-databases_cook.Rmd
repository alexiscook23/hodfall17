...
* A database management system (DBMS) is software that provides a way to interact with a database. It's how you read the data (like how we use R to read csv files)
...
req_text<-"Select * from flights" #the * means everything 

#Send query through connection
req<-dbSendQuery(con,req_text) #so when you send the request, it's just sitting there remotely in the database until you do the fetch command below

#Generate dataframe from results
req_df<-dbFetch(req,n=-1) #this means we don't want the first line cuz it's just text

#Good practice: clear request
dbClearResult(req) #after you're done, clear the request so it's not just sitting in the database and clogging stuff up
```
...
```{r}
dim(req_df) #this is asking what the dimensions are; rows first and then columns... we just want to know how big it is

head(req_df,20) #this still isn't big data because it can fit on our computer
```
...
req1_text<-"SELECT a.carrier, a.name, f.dep_delay 
            ...
            "
#the from flights thing means that we've renamed so that anytime we say "f." we're pulling from the flights file (same goes for airlines and a.)
#the join command means we're joining the airline and flight files by the carrier 
#the where part says only give us where the departure delay was more than 60 minutes
...
head(req1_df) #so this will show us each individual flight based on the three characteristics of carrier, name of airline, and time delayed in minutes. There's 3 columns but almost 27K rows since it's each individual delayed flight in 2013
...
delay_summary #so the summary shows the average departure delay in minutes based on airline, only for delays already longer than an hour 
```
...
dist_summary<-req1_df%>%
  group_by(name)%>%
  tally()%>% #the tally command just counts everything up
  arrange(-n) #so this will arrange based on number
...
#the and part just means that we match on multiple records
...
#seems to be a correlation that high wind gust actually lowers delays (probably lowers fog and increases visibility) unless it gets to really high winds and then delays go up a ton 
```

*Quick Exercise* Plot average delay by wind speed. 
```{r}
#had to create a new dataset so that we can have the wind speed and delays in one place
weather_summary2<-
  weather_df%>%
  group_by(wind_speed)%>%
  summarize(avg_delay=mean(dep_delay,na.rm=TRUE))
...
g1<-ggplot(data=weather_summary2,aes(x=wind_speed,y=avg_delay))
g1<-g1+geom_point()
g1
```
