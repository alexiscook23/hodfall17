...
---so basically you tell the computer how many groups you think there are (the k is how many numbers), and then it just puts that number of points randomly in the data and compares the mean distance between different clusters and keeps moving the different points until it minimizes the mean distance between the clusters. The job of the analyst is to figure out what those groups are based on once the computer shows you the groups. Also, it's not totally clear how many groups you should guess (there are some guidelines, but it's not black and white)
...
---something to think of is the size of your dataset; in something like this it should be fairly broad; like 3-7. In something massive like facebook, they probably have groups by the thousands so they can advertise effectively 
...
sf$purpose<-sf$q2_1
  ...
  #the dummy command turns categorical variables into a matrix; here we turned them into binary variables. you have to know what the responses were because the matrix will just give you numbers
```
...
c.test<-stepFlexclust(clus.data,k=2:7,nrep=20) #so this means try the cluster with 2 through 7 clusters, each 20 times, and see which fits best
...
plot(c1a) #this is a whisker plot, and the circles are outliers. still doesn't help us decide how many groups would be best; could be 3, 4, or 5 justifiably (4 has more outliers but smaller box)
...
table(c2$cluster) #in this, everyone should have the same answers (maybe they're different groups tho but same numbers)
...
rownames(pt)<-mycols
pt<-pt*100 #multiply everything by 100
pt<-round(pt,1) #round everything to the nearest decimal
```
...
# Summarizing group characteristics by cluster: this is really all you need to do! (the above steps were just to show you one by one how this works)
...
```{r}
sum2<-gather(sum1,-cluster,key=variable,value=value) #this basically just reorients the data so it goes from wide to long 
sum2
...
```{r}
g1 #the overall average for everyone are the dots; the bar plots are the average for that group
```


## Quick Exercise: Name these clusters for me
Group 1: Vloggers and Bougie Grandmas
Group 2: Sad Business Men
Group 3: Aunt Betsy visiting parents
Group 4: Strung out Disney parents
...
```
What this shows is that group 2 is the only group likely to have a problem while travelling; they were the only ones stat significant. (all of these are compared to group 1: the intercept)
```{r}
...
pred1<-pred1*100
pred1 #so this shows that group 2 (businessmen) are more likely to report a problem; 15% as compared to everyone else at 10%
```
...
Vloggers/Grandmas and Aunt Betsy are most likely to have an outstanding trip and businessmen are most likely to report a problem; Disney parents least likely to have an outstanding trip
