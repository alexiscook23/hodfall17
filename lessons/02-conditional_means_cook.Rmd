...
```
you can use the View function to see your different data sets by using a parenthesis with the file name 
...
```{r}
pd%>%filter(fips==047165)%>%select(per_capita_inc)

pd%>%filter(fips==047165)%>%select(coll_grad_pc)
...
In other words, 'mutate' creates a new variable; the thing first inside the () is the name and the = marks what it actually is
...
- We square them because if we just added them all up, we'd get zero (that's the definition of a mean)
-the double $ means "this is about to be math notation"
-if you want to pull an individual variable in a data set, use a single $ to show that (you'll see in line 145)
...
rmse_uncond_mean
```
means we're on average wrong by 7817 per capita income (pretty bad)
...
-ntil takes the dataset you're working with and divides it into however many sections you designate (we said 4 in line 161). So essentially we made a new variable called coll_grad_level using the division of college grad level in 4 quartiles

-group_by means you can group data based on the predictor you chose
<-pd> means you're overriding the normal data set to show given what you've chosen (your predictors)
...
$$\hat{Y}=(\bar{Y}|X=x) $$
-We have to reload these packages so that R knows the functions we're giving it
...
gg4<-gg
```
Because we just used two factors (college level and home value) and had 4 quartiles for each, we now have 16 total levels to predict income in the area. 
The "random" at the end of the mutate code means that if there's a tie between different counties, just break the tie randomly
