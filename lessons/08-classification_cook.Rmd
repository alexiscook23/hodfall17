...
#if there's no sentiment one way or the other, it gets a zero. Otherwise, it's a scale -4 to 4 where -4 will be very negative and 4 will be very positive
...
#these models are used by the military to predict terrorists, marketing companies to advertise to you correctly based on what you're likely interested in etc
```
...
plot(logit_roc)
```
So this model shows our sensitivity versus our specificity; to be a perfect model you'd have an area of one (it'd go straight up and straight over meaning it's perfectly sensitive and specific)... ours is .6; that's pretty bad. It goes like letter grades, so we basically got a D-
