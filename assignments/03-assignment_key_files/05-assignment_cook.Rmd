...
cex_cig<-cex[!(is.na(cex$cigarettes)),]
...
mod1<-lm(log(cigarettes+1)~
          ...
          bls_urbn+
           log(grocery+1),
           data=cex_cig,na.action="na.exclude")

summary(mod1)

exp(.7694)

cex_cig<-cex_cig%>%mutate(pred1=predict(mod1))

compare_data<-data.frame(cex_cig$cigarettes,cex_cig$pred1)%>%filter(is.na(cex_cig.pred1)==FALSE)
...
##can't figure out how to match up the data because cex_test has more columns than cex_cig since I omitted all the columns without data in the cigarette variable for the cex data
cex_test<-cex_test[!(is.na(cex$cigarettes)),]

cex_cig<-cex_cig%>%mutate(pred2=predict(mod1,newdata=cex_test))

rmse2<-with(cex_test,exp(rmse(log(cigarettes+1),pred2)));rmse2
